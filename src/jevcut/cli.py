...
#: The label sets every benchmark is scored against, the spec's first.
BENCH_LABELS = (
    ("v2 A", "eval/labels-v2"),
    ("v2 B", "eval/labels-v2-b"),
    ("v1 A", "eval/labels"),
    ("v1 B", "eval/labels-b"),
)
...
def cmd_bench(args: argparse.Namespace) -> int:
    """Score every saved version against every label set; write BENCHMARKS.md. No API."""
    from jevcut import evaluate
    ...
    if not entries:
        print("no benchmarks: save one with --snapshot NAME")
        return 1
    ...
    versions = "\n".join(
        f"| {a['name']} | `{a.get('commit') or '-'}` | {a.get('what', '')} |" for _, a in entries
    )
    doc = BENCH_DOC.format(versions=versions, tables="\n\n".join(sections))
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = sub.add_parser("bench", help="score every saved version on every label set (012)")
    p.add_argument("--out", default="BENCHMARKS.md")
    p.add_argument("--snapshot", metavar="NAME", help="first save the current runs as NAME")
